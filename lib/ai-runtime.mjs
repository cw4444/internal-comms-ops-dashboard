  ...
  try {
    response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        model,
        max_tokens: 4096,
        system: "You are an enterprise internal communications strategist and writer. Return valid JSON only. No markdown fences. Be concise, realistic and specific. Your output must include recommendation, drafts and insights for a large enterprise environment.",
        messages: [
          {
            role: "user",
...
export async function refineDraft(draftType, currentDraft, feedback, request) {
  ...
  const systemPrompt =
    "You are an enterprise internal communications editor. You will receive a draft, the operator's feedback, and the original request context. Return ONLY the improved draft text — no JSON wrapping, no markdown fences, no preamble. Apply the feedback precisely and preserve the original tone and structure unless the feedback says otherwise.";

  const userPrompt = JSON.stringify({
    task: "Refine this internal communications draft based on operator feedback.",
    draftType,
    currentDraft,
    ...
    constraints: [
      ...
      "Apply the operator feedback precisely.",
      "Keep the same format and structure unless feedback says otherwise.",
      "Do not add commentary or explanations — just the revised draft."
    ]
  });

  if (provider === "anthropic") {
    return refineWithAnthropic(systemPrompt, userPrompt);
  }

  return refineWithOpenAi(systemPrompt, userPrompt);
}

async function refineWithAnthropic(systemPrompt, userPrompt) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  const model = getPreferredModel("anthropic");
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 45_000);
  ...
  try {
    ...
    const payload = await response.json();
    if (!response.ok) {
      throw new Error(payload?.error?.message || "Anthropic refinement failed");
    }
    ...
    return { provider: "anthropic", model, refined: text };
  } finally {
    clearTimeout(timeoutId);
  }
}

async function refineWithOpenAi(systemPrompt, userPrompt) {
  const apiKey = process.env.OPENAI_API_KEY;
  const model = getPreferredModel("openai");
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 30_000);
  ...
  try {
    ...
    const payload = await response.json();
    if (!response.ok) {
      throw new Error(payload?.error?.message || "OpenAI refinement failed");
    }

    const text = extractOpenAiText(payload);
    return { provider: "openai", model, refined: text };
  } finally {
    clearTimeout(timeoutId);
  }
}
